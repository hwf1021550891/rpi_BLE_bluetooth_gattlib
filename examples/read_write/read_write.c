...
int main(int argc, char *argv[]) {
	...
	if (strcmp(argv[2], "read") == 0) {
		...
		unsigned char value_data[] = {		
			...
			0x8 , 0x1f, 0x40, 0x80, 0x7f, 0x0 , 0x0 , 0x8
		};
		memcpy(m_argument.value_data, value_data, sizeof(value_data));
		printf("Value to write: ");
		for (int i = 1; i < sizeof(m_argument.value_data); i++) {
			printf("%d ", m_argument.value_data[i]);
		}
		printf("\n");
	} else {
		usage(argv);
